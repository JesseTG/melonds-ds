...
os.execute(string.format('"%s" -E rm -rf "%s" "%s" "%s"', cmake, system_dir, save_dir, savestate_directory))
os.execute(string.format('"%s" -E make_directory "%s" "%s" "%s"', cmake, core_system_dir, save_dir, savestate_directory))
...
for _, f in ipairs(SYSTEM_FILES) do
    ...
    if sysfile ~= nil and sysfile ~= "" then
        ...
        local cmd = string.format('"%s" -E copy "%s" "%s/%s"', cmake, sysfile, core_system_dir, sysname)
