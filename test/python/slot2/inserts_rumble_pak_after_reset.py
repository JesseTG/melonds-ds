...
with prelude.builder().with_input(input_driver).with_options(options).build() as session:
    ...
    session.options.variables[b"melonds_slot2_device"] = b"rumble-pak"

    session.run()

    cart_type = get_gba_cart_type()
    assert cart_type == 0, f"Rumble Pak should not be inserted while a game is running"

    session.reset()

    cart_type = get_gba_cart_type()
    assert cart_type == 0x202, f"Rumble Pak should be inserted after a reset if enabled in the options"
